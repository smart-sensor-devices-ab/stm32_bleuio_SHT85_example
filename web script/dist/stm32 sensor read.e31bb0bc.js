...
document.getElementById('getdata').addEventListener('click', function () {
  document.getElementById('loader').innerHTML = 'Loading'; //set the BleuIO dongle into dual role

  my_dongle.at_dual().then(function () {
    // board id of the device that we are trying to get data from
    var boardID = '05084FA3'; //look for advertised data of with the board id

    my_dongle.at_findscandata(boardID, 4).then(function (x) {
      //split the advertised data from the respnse
      var advdata = x[x.length - 1].split(" ").pop(); //trim the advertised string to only get sensor response

      var result = advdata.split(boardID).slice(1).join(boardID); //get temperature and humidity value

      var temp = result.substring(0, 4);
      var hum = result.substring(4, 8); //convert from hex to decimal and device by 100

      temp = parseInt(temp, 16) / 100;
      hum = (parseInt(hum, 16) / 100).toFixed(1);
      document.getElementById('loader').innerHTML = '';
      document.getElementById('response').innerHTML = "Sensor ID : 05084FA3 <br/>\n          Temperature : ".concat(temp, " \xB0C<br/>\n          Humidity : ").concat(hum, " %rH<br/>");
    });
  });
});
...
},{}]},{},["C:/Users/Shuhad/AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","index.js"], null)
//# sourceMappingURL=/stm32%20sensor%20read.e31bb0bc.js.map